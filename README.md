...
> [!TIP]
> Packages are available for some major Linux distributions from [download.indii.org](https://download.indii.org), for [Arch Linux](https://aur.archlinux.org/packages/mendingwall) via the AUR, for [NixOS](https://search.nixos.org/packages?channel=unstable&query=mendingwall#show=mendingwall), and a Flatpak from [Flathub](https://flathub.org/en/apps/org.indii.mendingwall). You can also install from source following the instructions provided below.
